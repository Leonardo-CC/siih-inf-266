// services/farmaciaPerfilService.js
// ============================================================
// CAPA DE LOGICA - Edicion de perfil del farmaceutico.
// Campos editables: correo, telefono, contrasena.
// CAMPOS BLOQUEADOS: ci, rol, nombre, apellido, nro_licencia.
// ============================================================
...
export async function obtenerPerfilFarmaceutico(id_farmaceutico) {
  if (!id_farmaceutico) {
    return { ok: false, status: 400, mensaje: 'Falta el identificador del farmacéutico.' };
  }
  const { data, error } = await supabaseAdmin
    .from('farmaceutico')
    .select('id_farmaceutico, persona_id, nro_licencia, persona:persona_id (nombre, apellido, telefono)')
    .eq('id_farmaceutico', id_farmaceutico)
    .maybeSingle();

  if (error || !data) {
    return { ok: false, status: 404, mensaje: 'No se encontró el perfil del farmacéutico.' };
  }
...
export async function actualizarPerfilFarmaceutico(id_farmaceutico, payload = {}) {
  if (!id_farmaceutico) {
    return { ok: false, status: 400, errores: { general: 'Falta el identificador del farmacéutico.' } };
  }

  const perfilActual = await obtenerPerfilFarmaceutico(id_farmaceutico);
  if (!perfilActual.ok) {
  ...
  try {
    ...
    if (farm) {
      ...
      if (contrasenaHash) {
        const { error: errorUpdate } = await actualizarContrasenaPorPersona(farm.persona_id, contrasenaHash);
        if (errorUpdate) throw new Error(`Error actualizando contraseña: ${errorUpdate.message}`);
      }
    }

    const perfilActualizado = await obtenerPerfilFarmaceutico(id_farmaceutico);
    return {
      ok: true,
      status: 200,
...
export async function recuperarContrasenaFarmaceutico({ ci, correo, nuevaContrasena }) {
