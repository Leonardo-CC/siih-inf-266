...
const CAMPOS_BLOQUEADOS = ['ci', 'rol', 'nombre', 'apellido', 'nro_licencia'];
...
export default function MiPerfilFarmacia() {
  const usuario = obtenerUsuario();
  const [idFarmaceutico, setIdFarmaceutico] = useState(null);
  ...
  const [bloqueados, setBloqueados] = useState({ ci: '', rol: '', nombre: '', apellido: '', nro_licencia: '' });
  ...
  useEffect(() => {
    async function cargarPerfil() {
      ...
      try {
        let idFarm = usuario.id_farmaceutico;
        if (!idFarm) {
          const resId = await fetch('/api/farmaceutico/mi-id', {
            method: 'POST',
          ...
          idFarm = jsonId.id_farmaceutico;
          setIdFarmaceutico(idFarm);
        }

        const resPerfil = await fetch(`/api/farmaceutico/perfil?id_farmaceutico=${idFarm}`);
        ...
        setBloqueados({
          ...
          nombre: p.nombre || '',
          apellido: p.apellido || '',
          nro_licencia: p.nro_licencia || '',
        });
        setIdFarmaceutico(p.id_farmaceutico);
        setForm((prev) => ({
          ...prev,
          correo: p.correo || '',
          telefono: p.telefono || '',
  ...
  async function handleSubmit(e) {
    ...
    try {
      const res = await fetch('/api/farmaceutico/perfil', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  async function handleRecuperar(e) {
    ...
    try {
      const res = await fetch('/api/farmaceutico/recuperar-contrasena', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ci: recuperarForm.ci,
