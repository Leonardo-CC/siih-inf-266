...
const CAMPOS_BLOQUEADOS = ['ci', 'rol', 'nombre', 'apellido', 'nro_licencia', 'especialidad'];
...
export default function MiPerfilMedico() {
  ...
  const [bloqueados, setBloqueados] = useState({ ci: '', rol: '', nombre: '', apellido: '', nro_licencia: '', especialidad: '' });
  ...
  useEffect(() => {
    async function cargarPerfil() {
      ...
      try {
        let idMed = usuario.id_medico;
        if (!idMed) {
          const resId = await fetch('/api/medico/mi-id', {
            method: 'POST',
          ...
          idMed = jsonId.id_medico;
          setIdMedico(idMed);
        }

        const resPerfil = await fetch(`/api/medico/perfil?id_medico=${idMed}`);
  ...
  async function handleRecuperar(e) {
    ...
    try {
      const res = await fetch('/api/medico/recuperar-contrasena', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ci: recuperarForm.ci,
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                     value={`${formatearValor(bloqueados.nro_licencia)} · ${formatearValor(bloqueados.especialidad)}`.trim()}
                    disabled
                    className="w-full px-4 py-2.5 border border-slate-200 rounded-lg bg-slate-100 text-slate-500 cursor-not-allowed"
                  />
                  <p className="text-xs text-slate-400 mt-1"><IconoLock className="w-3 h-3 inline mr-1" /> Asignado por el hospital</p>
