...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (error || !data) return res.status(404).json({ ok: false, mensaje: 'No se encontró el perfil de médico.' });
    return res.status(200).json({ ok: true, id_medico: data.id_medico });
  } catch (err) {
